...
function [co,dt,nt,u,v,p,eu,ev,eP]=cavidadTesis
clc
tic
nx=50; % Número de nodos en el eje-x
ny=50; % Número de nodos en el eje-y
nit=10;
...
nu=0.005; % Viscosidad cinemática [m^2/s]
Re=(u_0*xmax)/nu; % Número de Reynolds
...
for i=1:length(x)
    for j=1:length(y)
        if sqrt((x(i)-xmax*0.25)^2+(y(j)-ymax*0.5)^2)<=r
            iv = cat(1, iv, [i,j]);
        end
    end
end
...
us(1,:)=0;
us(:,1)=0-us(:,2); % Establece la velocidad igual a la velocidad incial
us(nx,:)=0;
us(:,ny)=u_0-us(:,ny-1);   
vs(1,:)=0-us(2,:);
vs(:,ny)=0;
...
% ------------------------------------------------------------------------%    
    ...
    if k==1;
        plotFields(x,y,p,u,v,xmax,ymax,itr,dt);
    elseif k==50; 
        plotFields(x,y,p,u,v,xmax,ymax,itr,dt);
        k=0;
    end
    ...
    figure(6); clf
    q1=loglog(t,ev,'r'); hold on;
    q2=loglog(t,eu,'b'); hold on;
    q3=loglog(t,eP,'g'); hold on; xlabel('Tiempo [s]');ylabel('Error');
...
fprintf(' Min. Error U %.5f \n Min. Error V %.5f \n Min. Error P %.5f ',min(eu),min(ev),min(eP));
    function plotFields(x,y,p,u,v,xmax,ymax,itr,dt)
        figure(2)
